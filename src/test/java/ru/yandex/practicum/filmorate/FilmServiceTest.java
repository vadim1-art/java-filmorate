...
class FilmServiceTest {
    ...
    void createFilmWithDuplicateNameShouldThrowException() {
        filmService.create(new Film(null, "Title", "desc",
    ...
    void updateExistingFilmShouldChangeFields() {
        Film original = filmService.create(new Film(null, "Original", "Desc",
                LocalDate.of(2000, 1, 1), 100L, new Mpa(1, "G"), null));

        Film update = new Film(original.getId(), "Updated", "New desc",
                LocalDate.of(2001, 2, 2), 150L, new Mpa(2, "PG"), null);
    ...
    void updateFilmOwnNameDifferentCaseShouldNotThrowException() {
        Film original = filmService.create(new Film(null, "Inception", "Desc",
                LocalDate.now(), 100L, new Mpa(1, "G"), null));
        Film update = new Film(original.getId(), "INCEPTION", "Desc",
                LocalDate.now(), 100L, new Mpa(1, "G"), null);

        assertDoesNotThrow(() -> filmService.update(update));
        assertEquals("INCEPTION", filmService.getFilms().iterator().next().getName());
    }
}